...
cc_floor = cc.copy()
cc_floor[np.clip(np.nan_to_num(coh), 0, 1) < floor] = (
    0  # the conncomp_coh_floor post-mask
)
...
panels = [
    ...
    (
        ax[0, 1],
        lab1,
        f"coh_floor={floor}: cc>0 (dropped {dropped/max(mask.sum(),1)*100:.1f}%)",
        "RdYlGn",
        (0, 1),
    ),
...
for a, arr, title, cmap, vmm in panels + zpanels:
    im = a.imshow(
        arr, cmap=cmap, vmin=(vmm[0] if vmm else None), vmax=(vmm[1] if vmm else None)
    )
    a.set_title(title, fontsize=10)
    a.axis("off")
fig.suptitle(
    f"{frame}: conncomp_coh_floor cleans noisy percolation (cost_threshold can't)",
    fontsize=13,
)
fig.tight_layout()
...
print(
    f"{frame}: coh_floor={floor} dropped {dropped} px; noisiest window at {bij} (mean coh {best:.2f}) -> {out}",
    flush=True,
)
