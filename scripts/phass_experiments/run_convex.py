"""Run unwrap_convex on a scene. Companion to run_reuse.py.

Usage:  run_convex.py <scene>
  scene ∈ {nisar, pv}

Writes: <OUT>/<scene>_convex.npz
"""
...
scene = sys.argv[1]
print(f"[{scene}/convex] start", flush=True)
...
print(f"[{scene}/convex] shape={ig.shape}  valid={mask.sum():,}", flush=True)
...
print(f"[{scene}/convex] done in {elapsed:.1f}s", flush=True)
out_path = OUT_DIR / f"{scene}_convex.npz"
np.savez(out_path, unw=unw, k=k_int, elapsed=np.float32(elapsed))
print(f"[{scene}/convex] wrote {out_path}", flush=True)
