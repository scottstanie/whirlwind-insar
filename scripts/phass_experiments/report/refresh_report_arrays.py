...
A016OUT.mkdir(parents=True, exist_ok=True)
TAU = 2 * np.pi
...
def do_nisar():
    ...
    d = d - modal(d)
    print(
        f"NISAR default: {dt:.1f}s mainland match={100*np.mean(np.abs(d)<0.5):.2f}%",
        flush=True,
    )
...
def do_a016():
    ...
    _unw, _cc = ww.unwrap(
        igc,
        np.ascontiguousarray(coh, np.float32),
        16.0,
        np.ascontiguousarray(valid, bool),
    )
    ...
    a = a - modal(a)
    print(
        f"A_016 default (robust): {dt:.1f}s match={100*np.mean(np.abs(a)<0.5):.2f}%",
        flush=True,
    )
    np.savez_compressed(
        A016OUT / "a016_default_fixed.npz",
        unw=unw.astype(np.float32),
        prod=prod,
        pcc=pcc.astype(np.int32),
        coh=coh,
        mask=valid,
