...
def main() -> None:
    ...
    for j in sorted(top):
        print(
            f"{j:5d}  {ww_t[j]:8d}  {sn_t[j]:12d}  "
            f"{valid_per_col[j]/mask.shape[0]:.3f}      {mean_coh_col[j]:.3f}",
            flush=True,
        )
    ...
    print(
        ...
        f"{lowvalid} are <50% valid (data gaps)",
        flush=True,
    )
    ...
    r0 = max(0, r0)
    r1 = min(unw.shape[0], r0 + 1200)
    c0 = max(0, j - 120)
    c1 = min(unw.shape[1], j + 120)
    cr = lambda a: a[r0:r1, c0:c1]
    mk = cr(mask)
    fig, axes = plt.subplots(1, 4, figsize=(22, 8))
    panels = [
        ("input coherence", cr(np.where(mask, coh, np.nan)), "viridis", 0, 1),
        ("input wrapped", np.where(mk, cr(wrapped), np.nan), "twilight", -np.pi, np.pi),
        (
            "SNAPHU unwrapped",
            np.where(mk, cr(sunw) - np.nanmedian(sunw[region]), np.nan),
            "twilight",
            None,
            None,
        ),
        (
            "whirlwind unwrapped",
            np.where(mk, cr(unw) - np.nanmedian(unw[region]), np.nan),
            "twilight",
            None,
            None,
        ),
    ...
    fig.suptitle(
        f"NISAR thin vertical line @ col {j}: data artifact or whirlwind seam?",
        fontsize=13,
    )
    fig.tight_layout()
