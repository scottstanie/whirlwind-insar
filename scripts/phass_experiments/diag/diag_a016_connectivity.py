...
def main() -> None:
    ...
    print(
        f"A_016 valid={mask.sum():,} ({mask.mean():.1%})  correct={correct.sum():,}  drift={drift.sum():,}",
        flush=True,
    )
    ...
    print(
        f"  largest comp: {lc.sum():,} px; contains correct={np.sum(lc & correct):,}, drift={np.sum(lc & drift):,}",
        flush=True,
    )
    if np.sum(lc & correct) > 1000 and np.sum(lc & drift) > 1000:
        print(
            "  => SAME component spans correct+drift -> joined by a BRIDGE (within-component articulation).",
            flush=True,
        )
    else:
        print(
            "  => correct and drift are in DIFFERENT components -> separate-region leveling suffices.",
            flush=True,
        )
    ...
    print(
        ...
        f"(left avg {colcount[1500:1900].mean():.0f}, right avg {colcount[2300:2700].mean():.0f})",
        flush=True,
    )
    ...
    print(
        "\n  coherence-cut segmentation (keep pixel iff coh>=theta), components spanning frontier:",
        flush=True,
    )
    for theta in (0.3, 0.4, 0.5, 0.6, 0.7):
        ...
        print(
            f"    theta={theta}: {n2} comps, largest={sz2.max():,}px, "
            f"largest-spans-both={spans}, kept={hi.sum()/mask.sum():.0%} of valid",
            flush=True,
        )
