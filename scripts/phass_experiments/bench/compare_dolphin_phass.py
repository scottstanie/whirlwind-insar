"""Compute K-agreement of dolphin's PHASS output vs SNAPHU 9×9 on NISAR.

Also saves a 2-panel plot (dolphin PHASS vs SNAPHU 9×9 K-fields) to the
...
DOLPH = Path("/Volumes/WD_BLACK_SN7100_4TB/Documents/Learning/phass_experiments/dolphin_phass")
...
def main() -> None:
    ...
    print(f"dolphin PHASS conncomp coverage: {(dolph_cc>0).mean()*100:.2f}%, "
    ...
    print(f"dolphin PHASS K=match: {m0:.2f}%  (|dK|=1: {m1:.2f}%, "
          f"|dK|≥2: {m2:.2f}%, global offset {center:+d})")

    # Side-by-side plot vs SNAPHU
    import matplotlib.pyplot as plt
    panels = [
        ("SNAPHU 9×9", snaphu_k, snaphu_cc > 0),
    ...
    fig.suptitle("nisar: integer cycles K — dolphin PHASS vs SNAPHU 9×9",
                 fontsize=11)
    fig.tight_layout()
