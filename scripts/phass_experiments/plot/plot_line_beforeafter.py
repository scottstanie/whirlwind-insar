...
def main() -> None:
    ...
    jj = int(np.argmax(smooth))
    j = jj + 1
    rows = np.where(gh[:, jj])[0]
    rc = int(np.median(rows))
    r0 = max(0, rc - 180)
    r1 = min(before.shape[0], rc + 180)
    c0 = max(0, j - 90)
    c1 = min(before.shape[1], j + 90)
    print(
        f"smooth ghost line at col {j}, rows {rows.min()}..{rows.max()} ({gcol[jj]} px)",
        flush=True,
    )
    ...
    for ax in axes:
        ...
        ax.set_yticks([])
    fig.suptitle(
        f"NISAR ghost-line heal, smooth high-coh region @ col {j}", fontsize=14
    )
    fig.tight_layout()
