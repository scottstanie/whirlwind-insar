...
def main() -> None:
    ...
    def matchpct(k):
        d = (k - sk)[region]; d = d[np.isfinite(d)]
    ...
    panels = [
        ("SNAPHU 9×9 (reference)", sk_disp, None),
    ...
    def errmap(k):
        e = np.abs(k - sk); e[~region] = np.nan; return e
    axes[1, 0].axis("off")
    for ax, (title, k) in zip(axes[1, 1:], [("whole-image |dK| vs SNAPHU", kw), ("tiled |dK| vs SNAPHU", kt)]):
        im = ax.imshow(ds(errmap(k)), vmin=0, vmax=3, cmap="inferno", interpolation="nearest")
        ax.set_title(title, fontsize=11); ax.set_xticks([]); ax.set_yticks([])
        plt.colorbar(im, ax=ax, fraction=0.046, pad=0.02)

    fig.suptitle("NISAR no-Goldstein unwrap: whole-image vs tiled (K = round((unw−wrapped)/2π))", fontsize=13)
    fig.tight_layout()
