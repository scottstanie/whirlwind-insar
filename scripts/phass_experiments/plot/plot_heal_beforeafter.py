...
def main() -> None:
    ...
    print(
        f"smooth ghost col {j}, {int(gcol[jj])} px, rows {rows.min()}..{rows.max()}",
        flush=True,
    )
    r0 = max(0, rc - 180)
    r1 = min(off.shape[0], rc + 180)
    c0 = max(0, j - 90)
    c1 = min(off.shape[1], j + 90)
    ...
    for ax in axes:
        ...
        ax.set_yticks([])
    fig.suptitle(
        f"NISAR ghost-line heal (full pipeline, only heal toggled) @ col {j}",
        fontsize=14,
    )
    fig.tight_layout()
