...
def main() -> None:
    ...
    print(
        f"disagreeing pixels: {nz.sum():,} ({100*nz.sum()/reg.sum():.1f}% of cc>0)",
        flush=True,
    )
    print(
        f"whirlwind wrapped-residual p95 = {np.nanpercentile(np.abs(wres[mask]),95):.2e} rad (≈0 => valid unwrap)",
        flush=True,
    )
    vals, cnts = np.unique(diff_cyc[reg].astype(int), return_counts=True)
    top = sorted(zip(cnts, vals), reverse=True)[:6]
    print(
        "difference histogram (cycles): "
        + ", ".join(f"{v:+d}:{100*c/reg.sum():.0f}%" for c, v in top),
        flush=True,
    )
