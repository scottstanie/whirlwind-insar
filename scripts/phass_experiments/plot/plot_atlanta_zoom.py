...
def main() -> None:
    ...
    def ref_show():
        u = cr(refu).astype(np.float32)
        u[~mk] = np.nan
        return u - np.nanmedian(u[mk])

    def ww_show():
        u = cr(wwu).astype(np.float32)
        u[~mk] = np.nan
        return u - np.nanmedian(u[mk])

    wp = cr(wrapped).astype(np.float32)
    wp[~mk] = np.nan
    ...
    axes[0].imshow(
        wp, cmap="twilight", vmin=-np.pi, vmax=np.pi, interpolation="nearest"
    )
    ...
    for ax in axes:
        ...
        ax.set_yticks([])
    fig.suptitle(
        f"Atlanta crop [{r0}:{r0+sz}, {c0}:{c0+sz}] - is whirlwind carrying cycles?",
        fontsize=14,
    )
    fig.tight_layout()
