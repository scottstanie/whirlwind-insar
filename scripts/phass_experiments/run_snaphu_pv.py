...
doesn't, so generate one here. Single-tile (PV is small, 871×864).
"""
...
unw, cc = snaphu.unwrap(
    ig, coh, nlooks=5.0, cost="smooth",
    mask=mask.astype(np.uint8),
    nproc=os.cpu_count() or 1,
)
elapsed = time.perf_counter() - t0
print(f"snaphu done in {elapsed:.1f}s  n_components={int(cc.max())}  "
      f"coverage={(cc>0).mean()*100:.2f}%", flush=True)
