...
def main() -> None:
    ...
    prod = before["prod_unw"].astype(np.float64); pcc = before["prod_cc"]; mask = before["mask"]; coh = before["coh"]
    reg = mask & (pcc > 0) & np.isfinite(prod)

    def amb(unw):
        a = np.rint((unw.astype(np.float64) - prod) / TAU); return a - modal(a[reg])

    def m(unw):
        a = amb(unw)[reg]; a = a[np.isfinite(a)]; return 100 * np.mean(np.abs(a) < 0.5)
    ...
    ax[0, 0].set_title("coherence (large decorrelated body)"); fig.colorbar(im, ax=ax[0, 0], shrink=0.7)
    im = ax[0, 1].imshow(np.where(reg, pcc, np.nan)[s], cmap="tab10")
    ax[0, 1].set_title("production connectedComponents"); fig.colorbar(im, ax=ax[0, 1], shrink=0.7)
    im = ax[1, 0].imshow(ab[s], cmap="RdBu", vmin=-2, vmax=2)
    ax[1, 0].set_title(f"BEFORE: tile512 default ambiguity diff ({m(before['ww_unw']):.1f}% — winding + seam strip)")
    fig.colorbar(im, ax=ax[1, 0], shrink=0.7)
    im = ax[1, 1].imshow(aa[s], cmap="RdBu", vmin=-2, vmax=2)
    ax[1, 1].set_title(f"AFTER: gated multi-shift default ({m(after['unw']):.1f}% — strip gone, winding fixed)")
    fig.colorbar(im, ax=ax[1, 1], shrink=0.7)
    for a in ax.ravel():
        a.set_xticks([]); a.set_yticks([])
    p = OUT / "a016_before_after.png"
    fig.savefig(p, dpi=130); plt.close(fig)
    print(f"A_016 before={m(before['ww_unw']):.2f}%  after={m(after['unw']):.2f}%", flush=True)
