...
def main() -> None:
    ...
    print(f"[NISAR feather] {dt:.1f}s  mainland match={m0:.2f}%  |dK|>=2={m2:.2f}%", flush=True)
    print(f"  total vertical tears: no-anchor={int(old_t.sum()):,}  feather={int(new_t.sum()):,}", flush=True)
    ...
    amask = np.isfinite(phase) & np.isfinite(acoh) & np.isfinite(disp) & (acoh > 0) & (acoh < 1.0)
    awr = np.angle(np.exp(1j*np.where(amask, phase, 0.0))).astype(np.float32)
    aig = np.exp(1j*np.where(amask, phase, 0.0)).astype(np.complex64); aig[~amask] = 0
    acohw = np.clip(np.where(amask, acoh, 0), 0, 1).astype(np.float32)
    pr = disp*(4.0*np.pi/LAMBDA_S1)
    akref = np.round((pr - awr)/TAU); akref[~amask] = np.nan
    ...
    print(f"\n[Atlanta multilook=8 API] {adt:.1f}s  mainland match={am0:.2f}%  |dK|>=2={am2:.2f}%", flush=True)
