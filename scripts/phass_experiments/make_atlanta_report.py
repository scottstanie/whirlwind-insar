...
def ww_conncomp(unw, mask, stride):
    ...
    h, w = u.shape
    idx = np.full((h, w), -1, np.int64); idx[m] = np.arange(int(m.sum()))
    nn = int(m.sum())
    if nn == 0:
    ...
    counts = np.bincount(lab); order = np.argsort(counts)[::-1]
    remap = np.zeros(ncomp, np.int32)
    for new, old in enumerate(order, 1):
        remap[old] = new
    out = np.zeros((h, w), np.int32); out[m] = remap[lab]
    return out, ncomp


def comp_show(cc, ncolors=12):
    d = np.full(cc.shape, np.nan, np.float32); sel = cc > 0
...
def main() -> None:
    ...
    def kf(unw):
        k = np.round((unw - wrapped) / TAU); k[~mask] = np.nan
        d = (k - kref)[mainland]; return k - modal(d[np.isfinite(d)])

    kfine, kml = kf(fine), kf(ml8)
    def m_(k):
        d = (k - kref)[mainland]; d = d[np.isfinite(d)]
        return float((np.abs(d) < 0.5).sum())/d.size*100
    sd = kref.astype(np.float32).copy(); sd[~mask] = np.nan
    lo, hi = np.nanpercentile(sd[mask], [1, 99])
    ds = lambda a: a[::S, ::S]

    fig, axes = plt.subplots(2, 3, figsize=(19, 12))
    for ax, (t, k) in zip(axes[0], [
    ...
    def ph(u):
        x = u.astype(np.float32).copy(); x[~mask] = np.nan
        return x - np.nanmedian(u[mainland])
    pr = ph(refu)
    pw = ph(ml8)  # median-centering aligns the fields; no extra TAU*c (would double-count)
    plo, phi = np.nanpercentile(pr[mask], [1, 99])
    im = axes[1, 0].imshow(ds(pr), vmin=plo, vmax=phi, cmap="twilight", interpolation="nearest")
    axes[1, 0].set_title("OPERA unwrapped phase", fontsize=12)
    im2 = axes[1, 1].imshow(ds(pw), vmin=plo, vmax=phi, cmap="twilight", interpolation="nearest")
    axes[1, 1].set_title("whirlwind multilook-8 unwrapped phase", fontsize=12)
    for ax, im_ in [(axes[1, 0], im), (axes[1, 1], im2)]:
        ax.set_xticks([]); ax.set_yticks([]); plt.colorbar(im_, ax=ax, fraction=0.046, pad=0.02)
    wwcc, ncomp = ww_conncomp(ml8, mask, S)
    nref = int(np.unique(cc[cc > 0]).size)
    axes[1, 2].imshow(comp_show(wwcc), cmap="tab20", interpolation="nearest", vmin=0, vmax=12)
    axes[1, 2].set_title(f"whirlwind conncomp (largest-first)\nOPERA has {nref} comps", fontsize=12)
    axes[1, 2].set_xticks([]); axes[1, 2].set_yticks([])

    fig.suptitle("Atlanta S-1 OPERA: whirlwind fine-tiled FAILS on noise (26%); multilook-8 + tiled recovers it (97.7%, matches SNAPHU 97.9%)", fontsize=14)
    fig.tight_layout()
