...
def main() -> None:
    ...
    unw = d["ww_unw"].astype(np.float64); ig = d["ig"].astype(np.float64); coh = d["coh"].astype(np.float64)
    ...
    ax[0, 0].set_title("PRODUCTION unwrapped (SNAPHU)"); fig.colorbar(im, ax=ax[0, 0], shrink=0.6)
    im = ax[0, 1].imshow(np.where(reg, unw, np.nan), cmap="viridis", vmin=lo, vmax=hi)
    ax[0, 1].set_title("WHIRLWIND unwrapped (tile512) — valid, but right side wound +1"); fig.colorbar(im, ax=ax[0, 1], shrink=0.6)
    im = ax[0, 2].imshow(np.where(reg, a, np.nan), cmap="RdBu", vmin=-2, vmax=2)
    ax[0, 2].set_title("ambiguity diff ww-prod (cycles): the +1 winding region"); fig.colorbar(im, ax=ax[0, 2], shrink=0.6)

    # full frame: coherence + the DIFFERING (ww-only) cuts in red
    overlay(ax[1, 0], coh, ww_only, valid,
            "cuts ww made but SNAPHU did NOT (red), on coherence", zoom=(y0, y1, x0, x1))
    # zoom of the same
    ...
    rgba = np.zeros((*cz.shape, 4)); rgba[cz] = [1, 0, 0, 1]
    ax[1, 1].imshow(rgba, interpolation="nearest")
    ax[1, 1].set_title(f"ZOOM: ww-only cuts cross COHERENT pixels (bright)\n(box from left panel)")
    ax[1, 1].set_xticks([]); ax[1, 1].set_yticks([])
    # histogram of coherence on ww-only cuts vs scene
    chmin = np.minimum(coh[:, :-1], coh[:, 1:]); cvmin = np.minimum(coh[:-1, :], coh[1:, :])
    cc = np.concatenate([chmin[mh], cvmin[mv]])
    ax[1, 2].hist(coh[valid], bins=40, range=(0, 1), density=True, alpha=0.5, label="scene (valid)")
    ax[1, 2].hist(cc, bins=40, range=(0, 1), density=True, alpha=0.6, color="red", label="ww-only cut pixels")
    ax[1, 2].axvline(np.median(cc), color="red", ls="--", label=f"median {np.median(cc):.2f}")
    ax[1, 2].set_title("coherence: ww-only cuts vs scene\n(cuts sit on HIGH coherence = cost-suboptimal vs SNAPHU)")
    ax[1, 2].set_xlabel("coherence"); ax[1, 2].legend()

    p = OUT / "a016_cut_comparison.png"
    fig.savefig(p, dpi=120); plt.close(fig)
    print(f"ww-only cut pixels: {int(ww_only.sum()):,}; median coherence there = {np.median(cc):.3f} (scene median {np.median(coh[valid]):.3f})")
    ...
    def cutcoh(u):
        ...
        m = (valid[:, :-1] & valid[:, 1:] & (fh != 0)); m2 = (valid[:-1, :] & valid[1:, :] & (fv != 0))
        cc = np.concatenate([ch[m], cv[m2]])
        return cc.mean(), np.median(cc), 100 * np.mean(cc > 0.7)
    wm, wmd, w70 = cutcoh(unw); pm, pmd, p70 = cutcoh(prod)
    print(f"whirlwind cuts: mean coh={wm:.3f} median={wmd:.3f}  {w70:.0f}% through coh>0.7")
    print(f"production cuts: mean coh={pm:.3f} median={pmd:.3f}  {p70:.0f}% through coh>0.7")
    print(f"PLOT: {p}")
