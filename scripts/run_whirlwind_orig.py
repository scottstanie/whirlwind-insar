...
def main():
    ...
    for fr in args.frames:
        ...
        if not globs:
            print(f"{fr}: no h5 found in {GUNW_DIR}"); continue
        path = globs[0]
        ...
        np.savez_compressed(npz, unw=unw, prod_unw=prod_unw, prod_cc=prod_cc, mask=mask, coh=np.nan_to_num(coh).astype(np.float32))
        print(f"  saved -> {npz}")
