...
for lab in np.unique(prod_cc[in_c]):
    ...
    if m.sum() > 5000 and frac_majority < 0.95:
        mixed += 1
print(
    f"{frame}: production components >5k px that are INTERNALLY split (>5% off the "
...
print(
    f"{frame}: valid pixels with coh<0.3 (decorrelation bands/river): {lowcoh.sum()/mask.sum()*100:.1f}%"
)
...
show(
    ax[0, 2],
    prod_cc.astype(float),
    f"production conncomp ({int(prod_cc.max())})",
    cmap="tab20",
)
pu = np.where(both, prod, np.nan)
wu = np.where(both, unw, np.nan)
...
fig.suptitle(
    f"{frame}: A-025 river/bridging characterization (whirlwind per-comp ≈ 58%)"
)
fig.tight_layout()
