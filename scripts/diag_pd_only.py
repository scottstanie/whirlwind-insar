...
for mx in (8, 0):  # maxiter=0 = PD-only (no SSP); guarded — C++ has no no-progress break
    net=build()
    ...
    try:
        primal_dual(net, mx)
        signal.alarm(0)
        print(f"{frame}: {tag:20s} total_cost={net.total_cost()} balanced={net.is_balanced()} "
              f"total_excess={net.total_excess()} ({time.time()-t:.0f}s)",flush=True)
    except Timeout:
        print(f"{frame}: {tag:20s} DID NOT CONVERGE in 240s (PD alone cannot balance → relies on SSP)",flush=True)
