...
print(f"{'frame':7s} {'base%':>7s} {'bridged%':>9s} {'Δ':>6s} {'moved_px':>9s} {'ncomp':>6s} {'unw_s':>6s} {'br_s':>5s}", flush=True)
...
for h5path in h5s:
    ...
    pc_on = percomp_match(unw_on, prod_unw, wrapped, prod_cc, mask & np.isfinite(unw_on))
    rows.append((frame, pc_off, pc_on, moved))
    print(f"{frame:7s} {pc_off*100:7.2f} {pc_on*100:9.2f} {(pc_on-pc_off)*100:+6.2f} {moved:9d} "
          f"{int(cc.max()):6d} {t_unw:6.1f} {t_br:5.1f}", flush=True)

print("\n--- summary ---", flush=True)
regress = [r for r in rows if r[2] < r[1] - 1e-6]
improved = [r for r in rows if r[2] > r[1] + 1e-6]
noop = [r for r in rows if r[3] == 0]
print(f"improved: {[(r[0], round(r[1]*100,1), round(r[2]*100,1)) for r in improved]}", flush=True)
print(f"REGRESSED: {[(r[0], round(r[1]*100,1), round(r[2]*100,1)) for r in regress]}", flush=True)
