...
def percomp(u):
    ...
    amb = np.rint((u[in_c] - prod[in_c]) / tau); ccp = prod_cc[in_c]; ok = tot = 0
    for lab in np.unique(ccp):
        mm = ccp == lab; off = np.median(amb[mm]); ok += int((np.abs(amb[mm] - off) < 0.5).sum()); tot += int(mm.sum())
    return ok / tot


# Fine single-tile unwrap (the default) + its conncomp.
unw, cc = ww.unwrap(ig, coh_in, 16.0, mask)
unw = np.asarray(unw, np.float32); cc = np.asarray(cc)
print(f"{frame}: fine whirlwind per-comp={percomp(unw)*100:.1f}%  (ncc={int(cc.max())})", flush=True)
...
anchor = np.pad(anchor, ((0, max(0, m - anchor.shape[0])), (0, max(0, n - anchor.shape[1]))), mode="edge")[:m, :n]
print(f"{frame}: coarse ({mm}x{nn}, L={L}) coh>0.3 in river now {(cmask & (ccoh>0.3)).sum()/max(cmask.sum(),1)*100:.0f}% of coarse-valid", flush=True)
...
for snap_all in (False, True):  # gated vs ungated, for comparison
    ...
    for lab in np.unique(cc[cc > 0]):
        ...
        s = int(vals[cnts.argmax()]); frac = cnts.max() / len(votes)
        if s != 0 and (snap_all or frac >= THRESH):
            unw_fix[cc == lab] = unw[cc == lab] + tau * s
            nshift += 1
    tag = "ungated" if snap_all else f"gated@{THRESH}"
    print(f"{frame}: anchor-snap {tag:11s} per-comp={percomp(unw_fix)*100:.1f}%  (shifted {nshift} regions)", flush=True)
    if not snap_all:
...
for lab in range(1, nreg + 1):
    ...
    if s != 0:
        ...
        nshift += 1
print(f"{frame}: coh-region snap (coh>{COH_T}, {nreg} regions) per-comp={percomp(unw_cr)*100:.1f}%  "
      f"(shifted {nshift})", flush=True)
...
clab_up = np.pad(clab_up, ((0, max(0, m - clab_up.shape[0])), (0, max(0, n - clab_up.shape[1]))), mode="edge")[:m, :n]
...
print(f"{frame}: COARSE-region snap ({ncr} coarse regions, {big} big) per-comp={percomp(unw_cc)*100:.1f}%  "
      f"(shifted {nshift})", flush=True)
...
print(f"{frame}: oracle ceiling (snap fine cc -> production)  per-comp={percomp(unw_or)*100:.1f}%", flush=True)
