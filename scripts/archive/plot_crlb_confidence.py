"""Plots for the #58 CRLB work:
  ...
  Row 2 — CRLB cost vs coherence cost on IG 20240626_20240705, the case where the
          coherence cost is 50× worse (cut-rate 3.3e-2 vs 6.4e-4) — re-run live.
...
def main():
    ...
    a = np.load(EVAL / "after.npz"); ac = np.load(EVAL / "after_cor.npz")
    kp = kfield(a["unw"], w1, a["cc"] > 0)
    kc = kfield(ac["unw"], w1, ac["cc"] > 0)
    dk = np.round((ac["unw"] - a["unw"]) / TAU)
    dk = np.where((a["cc"] > 0) & (ac["cc"] > 0), dk - np.nanmedian(dk[(a["cc"] > 0) & (ac["cc"] > 0)]), np.nan)
    ...
    var2 = np.ascontiguousarray(np.nan_to_num(rd(IGDIR / f"crlb_{a2}.tif", np.float32) + rd(IGDIR / f"crlb_{b2}.tif", np.float32)), np.float32)
    cor2 = np.ascontiguousarray(np.clip(np.nan_to_num(rd(IGDIR / f"{stem2}.int.cor.tif", np.float32)), 0, 1), np.float32)
    ...
    panels = [
        ...
        ("IG0629 CRLB, .cor conf (4× faster)", kc, "viridis", None),
    ...
    kall = np.concatenate([p[1][np.isfinite(p[1])].ravel() for p in panels if p[1] is not None and p[2] == "viridis"])
    ...
    fig.suptitle("CRLB #58: .cor-as-confidence (row 1) + CRLB-cost vs coherence-cost (row 2)")
