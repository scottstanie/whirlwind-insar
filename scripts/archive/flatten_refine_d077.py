...
def main():
    ...
    ap.add_argument(
        "--size", type=int, default=0, help="0 = full frame; else center crop"
    )
    args = ap.parse_args()
    ...
    if args.size:
        ...
        prod_unw, prod_cc, coh, mask, wrapped = (
            a[sl].copy() for a in (prod_unw, prod_cc, coh, mask, wrapped)
        )
        M, N = s, s
    ...
    def match(field):
        ...
        return percomp_match(
            np.asarray(field, np.float32), prod_unw, wrapped, prod_cc, v
        )
    ...
    print(
        f"  tiled (anchor)        : {match(u_tiled)*100:6.2f}%  ({time.perf_counter()-t0:.1f}s)"
    )
    t0 = time.perf_counter()
    u_whole, _ = ww.unwrap(ig, coh, 16.0, mask, tile_size=big, tile_overlap=0)
    print(
        f"  whole-image (runaway) : {match(u_whole)*100:6.2f}%  ({time.perf_counter()-t0:.1f}s)"
    )

    # Hybrid: flatten by anchor, refine residual whole-image, add back.
    for name, anchor in [
        ("oracle=production", prod_unw.astype(np.float32)),
