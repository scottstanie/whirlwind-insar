...
# --- run Rust linear (now uses parity costs: 100× scale, both-invalid mask) ---
...
print(f"  Done in {t_linear:.1f}s, range [{np.nanmin(unw_linear):.1f}, {np.nanmax(unw_linear):.1f}]")
...
print(f"  Rust unwrap_linear (parity costs, maxiter=8):    {sc_linear*100:.2f}%  ({t_linear:.1f}s)")
...
largest_comp = max(np.unique(conn_prod[conn_prod > 0]),
                   key=lambda c: (conn_prod == c).sum())
