...
def real_palos_pair():
    if not HAS_RASTERIO:
        return None
    d = Path(
        "/Volumes/WD_BLACK_SN7100_4TB/Documents/Learning/capella/palos-verdes/opera-cslc-s1/Palos_Verdes_Landslides_D071/dolphin/interferograms"
    )
    if not d.exists():
        return None
...
def real_rosamond_pair():
    if not HAS_RASTERIO:
        return None
    d = Path(
        "/Volumes/WD_BLACK_SN7100_4TB/Documents/Learning/capella/rosamond/Rosamond_C13_RO43_eSM/insar_output/network_output/20250807_20250810"
    )
    if not d.exists():
        return None
    ...
    with rasterio.open(phs) as src:
        phase = np.nan_to_num(
            src.read(1)[1000 : 1000 + 512, 1000 : 1000 + 512].astype(np.float32),
            nan=0.0,
        )
...
def render_markdown(rows: list[Row]) -> str:
    ...
    lines.append(
        "Auto-generated by `scripts/bench.py`. Re-run after any perf change.\n"
    )
    lines.append(
        f"Host: {sys.platform}, Python "
    ...
    for scene, scene_rows in by_scene.items():
        ...
        for lib in lib_order:
            r = row_by_lib.get(lib)
            cells.append(
                f"{r.mpx_per_s():.2f}" if (r and np.isfinite(r.seconds)) else "n/a"
            )
        lines.append("| " + " | ".join(cells) + " |")
...
def main() -> int:
    ...
    def add_scene(name, igram, corr, nlooks, mask):
        ...
        for lib_name, fn in libs.items():
            ...
            rows.append(r)
            tag = (
                f"{r.seconds:7.3f}s  {r.mpx_per_s():5.2f} Mpx/s"
                if np.isfinite(r.seconds)
                else r.note
            )
            print(f" {tag}")
