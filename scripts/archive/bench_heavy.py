...
"""
    ...
    try:
        ...
        return Row(
            label=label, seconds=float("nan"), peak_rss_bytes=0, note=f"BAD STDOUT: {e}"
        )
    row = Row(label=label, seconds=data["seconds"], peak_rss_bytes=data["rss"])
    print(
        f"  {label:30s}  {row.seconds:8.2f} s   peak {fmt_bytes(row.peak_rss_bytes)}  "
...
def main():
    ...
    p.add_argument(
        "--only", default=None, help="comma-separated labels to run (substring match)"
    )
    p.add_argument(
        "--out", type=Path, default=Path(__file__).parent / "out" / "BENCH_HEAVY.md"
    )
    args = p.parse_args()
    ...
    if args.only:
        ...
        configs = [c for c in configs if any(w in c[0].lower() for w in wants)]
    ...
    # Markdown summary.
    lines = [
        f"# whirlwind-rs heavy benchmark ({H}x{W}, nlooks={nlooks})\n",
