...
def coherent_cut_rate(unw, wrapped, coh, valid):
    ...
    flowR = np.abs(
        np.round(
            (unw[:, 1:] - unw[:, :-1] - wrap_to_pi(wrapped[:, 1:] - wrapped[:, :-1]))
            / TAU
        )
    )
...
def main() -> None:
    ...
    print(
        f"COH_CUT_FLOOR = {COH_CUT_FLOOR:.1e}  ->  gate would {'FIRE (multi-shift, ~4x)' if fires else 'NOT fire (1x)'}"
    )
    print(
        "note: this is the FINAL (post-shift) result; if it fired, the base "
