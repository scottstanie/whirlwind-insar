...
def main() -> None:
    ...
    for e in edges:
        ...
        if ours.shape != theirs.shape:
            print(
                f"  shape mismatch on {ig_name}: ours {ours.shape}, theirs {theirs.shape}"
            )
            continue
    ...
    print(
        f"IGs with absolute RMS < 1 rad:    {summary['n_igs_with_rms_lt_1rad']}/{n_compared}"
    )

    if args.json is not None:
        args.json.parent.mkdir(parents=True, exist_ok=True)
        args.json.write_text(
            json.dumps({"summary": summary, "per_ig": per_ig}, indent=2)
        )
        print(f"\nWrote {args.json}")
