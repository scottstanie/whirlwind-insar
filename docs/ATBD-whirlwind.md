...
The field prior stays uniform, so the estimate remains maximum-likelihood ("probabilistic," not fully Bayesian).
...
Wrapped per-edge gradients are formed from complex conjugate products (`phase_dy = arg(igram[i+1,j]·conj(igram[i,j]))`, `phase_dx` analogously), then smoothed with a **separable box filter** (`box_filter_2d`, nearest-edge replication; Carballo's original paper used 5x5). The smoothed gradient $\hat\alpha$ estimates the underlying signal gradient by averaging over local phase noise. The window defaults to **7x7** but is exposed as `phase_grad_window = (parallel, perpendicular)` — SNAPHU's `KPARDPSI` / `KPERPDPSI` — where the *parallel* extent runs along the examined phase difference (so the vertical gradient is smoothed `parallel` rows × `perpendicular` cols and the horizontal gradient the transpose). A larger window steadies $\hat\alpha$ on high-fringe-rate deformation/topography at the cost of blurring it across wrap lines. The default `(7, 7)` reproduces the reference results exactly.
