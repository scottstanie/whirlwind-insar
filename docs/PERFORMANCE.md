...
> mask-disconnected regions to a coarse x8 anchor (`WHIRLWIND_NO_BRIDGE=1`
...
1. **Smooth interior or few residues** (top of every "noisy" row through 1024², all "clean" rows):
   Almost everything is the 7x7 box filter + per-arc cost evaluation; primal-dual is
...
| --------------------------------------------------------------------------------------------------------------------------------------------- | ------: | ---------: | -------: |
| 4096² γ=0.7 land + 35 % blob-shaped water mask (`heavy_scene.py --flavor noisy --low 0.7 --nlooks 10 --mask-fraction 0.35 --mask-kind blobs`) |  75.0 s | **0.54 s** | **139x** |
...
4x4 Lee-PDF LUT is fixed (~128 KiB) regardless of image size and rounds off.
