...
fn debug_asserts_or_wrong_on_noisy_ramp() {
    ...
    let (offsets, weights) = cost::compute_snaphu_smooth_costs(igram.view(), cor.view(), 4.0, None, cost::PhaseGradWindow::default());
...
fn negative_cycle_certificate_on_noisy_ramp() {
    ...
    for (seed, gamma_val, cycles, m) in [
        ...
        let residues = residue::compute(wrapped.view());
        let (offsets, weights) =
            cost::compute_snaphu_smooth_costs(igram.view(), cor.view(), 4.0, None, cost::PhaseGradWindow::default());
        let g = RectangularGridGraph::new(m + 1, n + 1);
        let nf = g.num_forward;
...
fn convex_quality_vs_baseline_on_noisy_ramp() {
    ...
    for (seed, gamma_val, cycles) in [(11_u64, 0.5_f32, 3.0_f32), (12, 0.7, 4.0)] {
        ...
        // unwrap_convex
        let unw_convex =
            whirlwind_core::unwrap_convex(igram.view(), cor.view(), 4.0, None, whirlwind_core::cost::PhaseGradWindow::default()).unwrap();
...
fn clean_steep_ramp_cost_under_wraps() {
    ...
    let (offsets, _weights) =
        cost::compute_snaphu_smooth_costs(igram.view(), cor.view(), 16.0, None, cost::PhaseGradWindow::default());
    let nf = offsets.len();
    let kstar_nonzero = offsets
        .iter()
        .filter(|&&o| ((o as f64 / 100.0).round() as i32) != 0)
    ...
    let unw = whirlwind_core::unwrap_convex(igram.view(), cor.view(), 16.0, None, whirlwind_core::cost::PhaseGradWindow::default()).unwrap();
