...
fn fmt_bytes(b: u64) -> String {
    const K: f64 = 1024.0;
    let f = b as f64;
    if f >= K * K * K { format!("{:.2} GiB", f / (K * K * K)) }
    else if f >= K * K { format!("{:.1} MiB", f / (K * K)) }
    else if f >= K { format!("{:.1} KiB", f / K) }
    else { format!("{} B", b) }
}
...
fn run_one(igram: &Array2<Complex32>, cor: &Array2<f32>, nlooks: f32) -> StageTimes {
    ...
    StageTimes {
        residue_ms, cost_ms, network_ms, primal_dual_ms, integrate_ms, total_ms,
...
fn analytic_mem_bytes(m: usize, n: usize) -> u64 {
    ...
    wrapped_phase
        + phase_dy_s + phase_dx_s + cor_dy + cor_dx
        + costs_temp
        + net_excess + net_potential + net_cost_fwd + net_saturated
        + sp_dist + sp_pred_arc + sp_pred_node + sp_source + sp_visited + heap_capacity
}

fn run_scene(label: &str, m: usize, n: usize, igram: Array2<Complex32>, cor: Array2<f32>, nlooks: f32) -> (StageTimes, u64, u64) {
    ...
    println!(
        ...
        label, size, mpx,
        st.residue_ms, st.cost_ms, st.network_ms, st.primal_dual_ms, st.integrate_ms, st.total_ms,
        throughput,
        fmt_bytes(analytic), fmt_bytes(rss_after.saturating_sub(rss_before)),
    );
...
fn main() {
    ...
    println!("Times in milliseconds. Peak RSS via `getrusage(RUSAGE_SELF)`; the RSS column\n\
              shows the **delta** to the prior call so each row gives a real per-image cost.\n");

    println!("## Per-stage timing + memory\n");
    println!("| Scene                    | size      |    Mpx | residue |   cost |    net |      pd | integ. |  total | Mpx/s  | analytic mem | ΔpeakRSS |");
    println!("|--------------------------|-----------|-------:|--------:|-------:|-------:|--------:|-------:|-------:|-------:|-------------:|---------:|");
    ...
    let sizes_vn: &[usize] = if include_huge { &[256, 512, 1024, 2048] } else { &[256, 512, 1024] };
    for &n in sizes_vn {
    ...
    println!("| Scene                    | size      | residues | pd iters | dijkstra |  augment | potential | ssp iters |");
    println!("|--------------------------|-----------|---------:|---------:|---------:|---------:|----------:|----------:|");
    for (label, n, (st, _, _)) in &all {
        ...
        println!(
            ...
            scene, size, st.pd_residues, st.pd_iters,
            st.pd_dijkstra_ms, st.pd_augment_ms, st.pd_potential_ms,
            st.pd_ssp_iters,
        );
    }
}
